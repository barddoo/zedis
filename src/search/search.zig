...
pub fn search(
    ...
    try collectCandidates(allocator, index, &q.filter, &candidates);

    var results: std.ArrayListUnmanaged(Result) = .empty;
    errdefer results.deinit(allocator);

    if (q.knn) |knn| {
        const field_idx = index.fieldIndex(knn.field) orelse return error.UnknownField;
        const vi = index.getVector(field_idx) orelse return error.UnknownField;
        const blob = params.?.get(knn.vec_param) orelse return error.MissingParam;

        var knn_res = try vi.knnFiltered(blob, knn.k, &candidates);
        defer knn_res.deinit(allocator);
        for (knn_res.items) |kr| {
            const doc_id = findDocId(index, kr.doc_id) orelse continue;
        ...
        try collectTerms(allocator, index, &q.filter, &terms);

        var it = candidates.iterator();
        while (it.next()) |e| {
            const internal_id = e.key_ptr.*;
            const doc_id = findDocId(index, internal_id) orelse continue;
            const score = scoreDoc(index, terms.items, internal_id);
            try results.append(allocator, .{
                .doc_id = doc_id,
        ...
        try applySort(index, &results, opts);
    }
...
fn collectCandidates(allocator: Allocator, index: *const SearchIndex, node: *const Node, set: *DocSet) !void {
    switch (node.*) {
        .all => {
        ...
        .term => |t| {
            for (index.fields, 0..) |f, i| {
                ...
                if (index.getInverted(@intCast(i))) |ii| {
        ...
        .phrase => |p| {
            ...
            for (tokenizer.tokens.items) |word| {
                ...
                if (set.count() == 0) {
                    ...
                    try intersectWith(allocator, set, &word_set);
                }
            }
        },
        .field_term => |ft| {
            const field_idx = index.fieldIndex(ft.field) orelse return error.UnknownField;
            const ii = index.getInverted(field_idx) orelse return error.UnknownField;
            if (ii.postings(ft.term)) |posts| {
                for (posts) |p| try set.put(allocator, p.doc_id, {});
            }
        },
        .numeric => |n| {
            const field_idx = index.fieldIndex(n.field) orelse return error.UnknownField;
            const ni = index.getNumeric(field_idx) orelse return error.UnknownField;
        ...
        .tag => |t| {
            const field_idx = index.fieldIndex(t.field) orelse return error.UnknownField;
            const ti = index.getTag(field_idx) orelse return error.UnknownField;
            for (t.tags) |tag| {
                if (ti.docIds(tag)) |ids| {
                    var it = ids.iterator();
                    while (it.next()) |e| try set.put(allocator, e.key_ptr.*, {});
                }
            }
        ...
        .conjunction => |children| {
            ...
            try intersectWith(allocator, &all, set);
            var out = all;

            for (children) |*child| {
                var child_set: DocSet = .empty;
                defer child_set.deinit(allocator);
                try collectCandidates(allocator, index, child, &child_set);
                try intersectWith(allocator, &out, &child_set);
            }
        ...
        .disjunction => |children| {
            for (children) |*child| try collectCandidates(allocator, index, child, set);
        },
        .not => |child| {
            var child_set: DocSet = .empty;
            defer child_set.deinit(allocator);
...
fn intersectWith(allocator: Allocator, a: *DocSet, b: *const DocSet) !void {
    var it = a.iterator();
...
fn collectTerms(allocator: Allocator, index: *const SearchIndex, node: *const Node, out: *std.ArrayListUnmanaged(QueryTerm)) !void {
    switch (node.*) {
        ...
        .disjunction => |children| {
            for (children) |*child| try collectTerms(allocator, index, child, out);
        },
        .conjunction => |children| {
            for (children) |*child| try collectTerms(allocator, index, child, out);
        },
...
fn scoreDoc(index: *const SearchIndex, terms: []const QueryTerm, internal_id: u64) f64 {
    var score: f64 = 0;
    for (terms) |t| {
        if (t.field) |field_idx| {
            if (index.getInverted(field_idx)) |ii| {
                score += ii.bm25(internal_id, ii.docFreq(t.text), ii.tf(internal_id, t.text));
            }
        } else {
            for (index.fields, 0..) |f, i| {
                if (f.field_type != .text) continue;
...
fn findDocId(index: *const SearchIndex, internal_id: u64) ?[]const u8 {
    var it = index.docs.iterator();
    while (it.next()) |e| {
...
fn scoreLessThan(ctx: SortCtx, a: Result, b: Result) bool {
    return if (ctx.ascending) a.score < b.score else a.score > b.score;
}

fn sortKeyLessThan(ctx: SortCtx, a: Result, b: Result) bool {
...
fn applySort(index: *const SearchIndex, results: *std.ArrayListUnmanaged(Result), opts: *const SearchOptions) !void {
    if (opts.sort_by) |sb| {
        if (std.ascii.eqlIgnoreCase(sb, "score")) {
            const ctx: SortCtx = .{ .ascending = opts.sort_asc };
            std.mem.sort(Result, results.items, ctx, scoreLessThan);
            return;
        }
        const field_idx = index.fieldIndex(sb) orelse return error.UnknownField;
        for (results.items) |*r| {
            const doc = index.getDocument(r.doc_id) orelse continue;
            r.sort_key = docFieldValue(doc, field_idx);
        }
        const ctx: SortCtx = .{ .ascending = opts.sort_asc };
        std.mem.sort(Result, results.items, ctx, sortKeyLessThan);
        return;
    }
...
fn docFieldValue(doc: *const @import("index.zig").Document, field_idx: u16) ?f64 {
